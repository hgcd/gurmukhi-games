...
function startRound() {
    ...
    // Always show audio question with Gurmukhi options
    showAudioQuestion();
    generateGurmukhiOptions();
...
function showAudioQuestion() {
    audioQuestion.classList.remove('d-none');
    
    correctAnswer = currentQuestion.punjabi_gurmukhi;
    
    // Setup audio
    setupAudio(currentQuestion);
...
// Setup audio for the current question
function setupAudio(word) {
    ...
    if (word && word.audio) {
        ...
        // Auto-play the audio when question is shown
        setTimeout(() => {
            try { 
                audioElement.currentTime = 0; 
                audioElement.play(); 
...
// Generate Gurmukhi options
function generateGurmukhiOptions() {
    ...
    updateOptionsDisplay(false); // false indicates text-only options
}
...
function updateOptionsDisplay(isImageOptions = false) {
    for (let i = 1; i <= 5; i++) {
        ...
        if (option) {
            // Simple text display for Gurmukhi options
...
function selectAnswer(answer, button) {
    ...
    // Show the image of the correct word
    showCorrectWordImage();
...
// Show the image of the correct word after answer is revealed
function showCorrectWordImage() {
    const questionImage = document.getElementById('question-image');
    const imageContainer = document.getElementById('word-image-container');
    
    if (questionImage && currentQuestion.image && imageContainer) {
